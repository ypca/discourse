...
export default Ember.Component.extend(AddArchetypeClass, Scrolling, {
  ...
  didInsertElement() {
    ...
    this.appEvents.on("post:highlight", this, this._highlightPost);
    ...
    if (this.site.mobileView) {
      this.appEvents.on("topic:scrolled", this, this.mobileScrollGuard);
      // used to animate header contents on scroll
      this.appEvents.on("header:show-topic", this, this._showTopic);
      this.appEvents.on("header:hide-topic", this, this._hideTopic);
    }
  },
  ...
  willDestroyElement() {
    ...
    this.appEvents.trigger("header:hide-topic");
    this.appEvents.off("post:highlight", this, this._highlightPost);
    // mobile scroll logo clean up.
    if (this.site.mobileView) {
      this.appEvents.off("topic:scrolled", this, this.mobileScrollGuard);
